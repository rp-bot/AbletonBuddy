...
async def classify_user_input(
    ...
    guidance = (
        ...
        "- DEVICE: Device-level operations (list devices, get/set parameters, query parameter metadata for already-loaded devices).\n"
        "- DEVICE_LOADER: Browser-powered device search/loading and loader cache management (searching for sounds/plugins, loading them onto the selected track, rebuilding/checking the cache, test load).\n"
        "- COMPOSITION: MIDI content generation (creating musically coherent melodies, chord progressions, drum patterns in MIDI clips).\n\n"
        ...
        "- If it's about CREATING an EMPTY clip (e.g., 'create an empty clip', 'create a blank clip', 'make a new empty clip'): include CLIP_SLOT (and TRACK if the request also references track-level context).\n"
        ...
        "- Key distinction: 'create an empty clip' = CLIP_SLOT, 'edit clip X' or 'launch clip X' = CLIP.\n"
        "- If it's about triggering/managing scenes: include SCENE (and SONG if transport context is implicated).\n"
        "- If it's about parameters of an existing device already on a track: include DEVICE (and TRACK if scoped to a track).\n"
        "- If it's about searching Ableton's browser, loading/adding a new device/instrument/effect, or managing the loader cache (search/load/rebuild/test/cache size): include DEVICE_LOADER (often along with VIEW when a specific track should be selected first).\n"
        "- If it's about the Live application lifecycle, version, or logs: include APPLICATION.\n"
        "- If it's about GENERATING musical content (melodies, chord progressions, drum patterns): include COMPOSITION ONLY (do NOT include CLIP_SLOT, as COMPOSITION handles both creating the clip and adding the content).\n"
        "- Key distinction: 'create a melody', 'generate a chord progression', 'make a drum pattern' = COMPOSITION ONLY. 'create an empty clip' = CLIP_SLOT.\n\n"
        "Edge cases:\n"
        ...
        "- 'Load Operator on track 3' or 'Search for a warm pad sound and add it' => DEVICE_LOADER (plus VIEW when selecting tracks is necessary).\n"
        "- 'Create a melody in C major' or 'Generate a chord progression' or 'Make a drum pattern' => COMPOSITION ONLY (not CLIP_SLOT, as COMPOSITION creates the clip with content).\n"
        "- 'Create a hip-hop melody 4 bars long' => COMPOSITION ONLY (not CLIP_SLOT).\n\n"
        "Only return labels from this set: APPLICATION, SONG, VIEW, TRACK, CLIP_SLOT, CLIP, SCENE, DEVICE, COMPOSITION.\n"
