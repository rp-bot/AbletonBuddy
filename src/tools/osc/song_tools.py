"""
CrewAI tools for AbletonOSC Song API - comprehensive session control.
"""
...
from ableton_agent.tools.osc.client import OSCClient


class OSCQueryInput(BaseModel):
    query_type: str = Field(..., description="Query type: tempo, is_playing, track_names, num_tracks, metronome, current_song_time, loop, signature_numerator, signature_denominator, session_record, arrangement_overdub, back_to_arranger, can_redo, can_undo, clip_trigger_quantization, groove_amount, midi_recording_quantization, nudge_down, nudge_up, punch_in, punch_out, record_mode, session_record_status, song_length")
    params: Optional[str] = Field(
        default=None, description="Optional comma-separated params for the query")


class OSCCommandInput(BaseModel):
    command_type: str = Field(..., description="Command: start_playing, stop_playing, continue_playing, set_tempo, set_current_song_time, set_signature_numerator, set_signature_denominator, trigger_session_record, set_session_record, set_arrangement_overdub, set_metronome, set_loop, set_loop_start, set_loop_length, create_midi_track, create_audio_track, create_scene, undo, redo, tap_tempo, capture_midi, create_return_track, delete_scene, delete_return_track, delete_track, duplicate_scene, duplicate_track, jump_by, jump_to_next_cue, jump_to_prev_cue, stop_all_clips, set_back_to_arranger, set_clip_trigger_quantization, set_groove_amount, set_midi_recording_quantization, set_nudge_down, set_nudge_up, set_punch_in, set_punch_out, set_record_mode")
    value: Optional[Union[str, int, float]] = Field(
        default=None, description="Value for the command (e.g., tempo)")
    additional_params: Optional[str] = Field(
        default=None, description="Additional comma-separated params if needed")


class AbletonSongQueryTool(BaseTool):
    name: str = "Query Ableton Live Song"
    description: str = (
    ...
    args_schema: Type[BaseModel] = OSCQueryInput

    def _run(self, query_type: str, params: Optional[str] = None) -> str:
        osc = OSCClient()
        query_map = {
...
class AbletonSongCommandTool(BaseTool):
    name: str = "Control Ableton Live Song"
    description: str = (
        "Execute Song API commands via AbletonOSC. "
        "Use command_type like 'set_tempo', 'start_playing', 'stop_playing', etc."
    )
    args_schema: Type[BaseModel] = OSCCommandInput

    def _run(self, command_type: str, value: Optional[Union[str, int, float]] = None, additional_params: Optional[str] = None) -> str:
        osc = OSCClient()
        command_map = {
            # Playback control
        ...
        result = osc.send_and_wait(
            address, args if args else None, timeout=1.0)
        if result is None or result == 'OK':
            return f"Command executed: {command_type}" + (f" with value {value}" if value is not None else "")
        return f"Command {command_type} result: {result}"


class AbletonTestInput(BaseModel):
    """Input schema for AbletonTestTool."""
    pass  # No parameters needed for test connection


class AbletonTestTool(BaseTool):
    name: str = "Test Ableton Connection"
    description: str = "Test the AbletonOSC connection by calling /live/test"
    args_schema: Type[BaseModel] = AbletonTestInput
