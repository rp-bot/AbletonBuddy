...
% Left Box (Frontend): React UI $\rightarrow$ HTTP POST (``Create a hiphop melody'')
% Middle Box (Flask/Marvin Core):
% $\downarrow$ Disambiguator Node
% $\downarrow$ Router (Diamond Shape) splits into $\rightarrow$ [Composition Agent] / [Mixing Agent] / [Arrangement Agent]
% $\downarrow$ Agents consult [Knowledge Priors DB]
% Right Box (Control): $\rightarrow$ OSC Protocol $\rightarrow$ Ableton Live
% Bottom Loop: $\leftarrow$ OSC Feedback $\leftarrow$ Summary Agent $\rightarrow$ React UI
